...
TEST(LibraryTest, good_weather) {
  // Given
  PageMock page_mock;
  EXPECT_CALL(page_mock, refresh()).WillOnce(Invoke([]() {
    std::cout << "PageMock::refresh()" << std::endl;
  }));
...
TEST(LibraryTest, get_pointer_value) {
  ...
  auto print_parameters = [](void *type, void *secret_p) {
    std::cout << "type=" << *static_cast<int *>(type)
              << " secret=" << *static_cast<int *>(secret_p) << std::endl;
  };
  ...
  EXPECT_CALL(page_mock, insert(_, _))
      .WillOnce(
          DoAll(Invoke(print_parameters), Invoke(get_parameter), Return(0)));
