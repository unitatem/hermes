...
TEST(VisitorTest, good_weather) {
    ...
    std::vector<std::reference_wrapper<Visitor>> visitors = {
        xml_report_visitor, json_report_visitor};
    ...
    // Then
    EXPECT_EQ(visitors[0].get().report,
              "XmlReportVisitor > CityNode\nXmlReportVisitor > TownNode\n");
    EXPECT_EQ(visitors[1].get().report,
              "JsonReportVisitor > CityNode\nJsonReportVisitor > TownNode\n");
}
